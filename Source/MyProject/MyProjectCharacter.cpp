...
class TinyClientSpawnActor {};
template<>
TinyClientSpawnActor* UWorld::SpawnActor(UClass* Class, const FActorSpawnParameters& SpawnParameters)
{
	if (GUndo)
	{
		ModifyLevel(CurrentLevel);
	}
	CurrentLevel->Actors.Add(CurrentActor);
	CurrentLevel->ActorsForGC.Add(CurrentActor);

	CurrentActor->RegisterAllComponents();
	FComponentInstanceDataCache datacache(CurrentActor);
	CurrentActor->FinishSpawning(CurrentActor->GetTransform(), true, &datacache);
	// Broadcast notification of spawn
	OnActorSpawned.Broadcast(CurrentActor);

#if WITH_EDITOR
	if (GIsEditor)
	{
		GEngine->BroadcastLevelActorAdded(CurrentActor);
	}
#endif
...
void AMyProjectCharacter::BeginPlay()
{
	...
	if (ExportPackage)
	{
		...
		for (auto actor : SpawnActors)
		{
			...
			world->SpawnActor<TinyClientSpawnActor>(nullptr);
		}
	}
}
...
void AMyProjectCharacter::DupActor(AActor* Obj)
{
	FActorSpawnParameters SpawnParams;
	SpawnParams.Template = Obj;

	GetWorld()->SpawnActor<AActor>(CurrentActor->GetClass(), FVector(0.0f,0.0f,0.0f), CurrentActor->GetActorRotation(), SpawnParams);
}
